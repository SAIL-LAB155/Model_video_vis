...
class PoseEstimator(object):
    def __init__(self, pose_cfg, pose_weight):
        ...
        if opt.pose_backbone == "seresnet101":
            from src.pose_model.seresnet.FastPose import InferenNet_fast as createModel
            self.pose_model = createModel(4 * 1 + 1, "", pose_weight, cfg=pose_cfg)
        elif opt.pose_backbone == "mobilenet":
            from src.pose_model.mobilenet.MobilePose import createModel
            self.pose_model = createModel(cfg=opt.pose_cfg)
        ...
        inf_time = get_inference_time(self.pose_model, height=opt.input_height, width=opt.input_width)
        ...
        self.batch_size = opt.pose_batch
    ...
    def process_img(self, inps, boxes, pt1, pt2):
        ...
        preds_hm, preds_img, preds_scores = getPrediction(
            hm, pt1, pt2, opt.input_height, opt.input_width, opt.output_height, opt.output_width)
